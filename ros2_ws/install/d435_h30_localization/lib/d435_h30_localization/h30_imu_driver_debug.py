...
class H30ImuDriverDebug(Node):
    def __init__(self):
        super().__init__('h30_imu_driver_debug')
        ...
        self.get_logger().info(f"Debug mode: H30 IMU Driver started on {self.serial_port} @ {self.baud_rate}")
    ...
    def print_stats(self):
        self.get_logger().info(f"Stats: Accel={self.accel_count}, Gyro={self.gyro_count}, Quat={self.quat_count}, Published={self.packets_received}")
        self.get_logger().info(f"Current Accel: {self.last_acceleration}")
        self.get_logger().info(f"Current Gyro: {self.last_angular_velocity}")
        if self.got_quat:
            self.get_logger().info(f"Current Quat: {self.last_orientation}")
        ...
        if self.ser and self.ser.is_open:
            try:
                if self.ser.in_waiting > 0:
                    ...
                    self.get_logger().debug(f"Received {len(data)} bytes: {data.hex()}")
                    self.buffer += data
